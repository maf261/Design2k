...
```{r}
url <- "https://raw.githack.com/maf261/Design2k/master/Tab3.txt"
...
text(aux$x, aux$y, rownames(aux$y), cex = 0.8, pos = 2)
```

Através do gráfico acima, identificamos os efeitos mais discrepantes. As que ficaram no centro, podem ser consideradas como nulas. Portanto, podemos ajustar agora o melhor modelo. Vejamos ainda o gráfico de Pareto.
...
Ou seja, rejeitamos a $10\%$ a hipótese de que o modelo linear é adequado (ou de que não existe falta de ajuste). De outra forma, podemos proceder:
...
Nesse caso, rejeitamos a hipótese de que essas médias são iguas e concluimos que há falta de ajuste, e que o modelo linear não é adequado. Assim, podemos redefinir o modelo com esse termo

```{r}
mf <- lm(Y ~ (X1*X2*X3) + lof, data = dados)
anova(mf)
```
