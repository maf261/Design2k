...
```{r}
url <- "https://raw.githack.com/maf261/Design2k/master/Tab2.txt"
...
Através do gráfico acima, vemos que os efeitos mais discrepantes são de X2 e das interações X1:X2 e X1:X2:X3. As que ficaram no centro, podem ser consideradas como nulas. Portanto, para sermos conservadores, podemos ajustar agora um modelo considerando apenas os fatores X1, X2, X3 e as interações X1:X2 e X1:X2:X3 .
...
```{r}
m01 <- lm(Y ~ (X1*X2*X3)-X1:X3-X2:X3, data = dados)
anova(m01)
```
...
Como as interações e os fatores X2 e X3 são não significativos, então também podemos retira-los do modelo. No entanto, o efeito da interação X1:X2:X3 foi inicialmente indicado como forte. Por isso, podemos agora atualizar o modelo e na dúvida, manter a interação X1:X2:X3 para avaliação, e por consequência devemos manter também X2 e X3 pelo princípio da marginalidade.

```{r}
mf <- lm(Y ~ (X1*X2*X3)-X1:X3-X2:X3,data=dados)
anova(mf)
```
